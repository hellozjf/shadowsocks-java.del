        ...
        synchronized (flowStatisticsDetailRepository) {
            synchronized (flowSummaryRepository) {
                synchronized (userInfoRepository) {
                    ...
                    // 删除用户的时候，要将FlowStatisticsDetail和FlowSummary表里面的数据也要删除
                    flowStatisticsDetailRepository.deleteAllByServerPort(userInfo.getPort());
                    flowSummaryRepository.deleteAllByUserInfoId(userInfo.getId());
